...
export function DemoStrip() {
  // Demos with audio start MUTED by default — the bootstrap inside the iframe
  // also starts muted, so initial state is coherent (silent + showing "muted" icon).
  const [muted, setMuted] = useState<Set<string>>(
    () => new Set(DEMOS.filter((d) => d.hasAudio).map((d) => d.file)),
  );
  const iframeRefs = useRef<Record<string, HTMLIFrameElement | null>>({});

  const sendMute = useCallback((file: string, shouldMute: boolean) => {
    const ifr = iframeRefs.current[file];
    try {
      ifr?.contentWindow?.postMessage({ __mute: shouldMute }, "*");
    } catch {
      /* ignore */
    }
  }, []);

  // Re-apply mute state to iframes whenever `muted` changes.
  useEffect(() => {
    Object.keys(iframeRefs.current).forEach((f) => {
      sendMute(f, muted.has(f));
    });
  }, [muted, sendMute]);

  // Listen for the iframe's readiness announcement and push current state.
  useEffect(() => {
    const onMsg = (e: MessageEvent) => {
      const data = e.data;
      if (!data || typeof data !== "object" || !data.__muteReady) return;
      // Match against any iframe whose contentWindow matches the source.
      for (const [file, ifr] of Object.entries(iframeRefs.current)) {
        if (ifr?.contentWindow === e.source) {
          sendMute(file, muted.has(file));
          break;
        }
      }
    };
